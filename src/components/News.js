import React, { Component } from 'react'
import NavBar from './NavBar'
import NewItems from './NewItems'
// import './News.css'
export class News extends Component {
    rides1 = [{"id":956,"origin_station_code":1,"station_path":[53,67,74,89],"destination_station_code":92,"date":"03/21/2022 11:02 PM","map_url":"https://picsum.photos/200","state":"Arunachal Pradesh","city":"Pasighat"}];
    rides2 = [{"id":654,"origin_station_code":1,"station_path":[53,67,74,89],"destination_station_code":92,"date":"03/21/2022 11:02 PM","map_url":"https://picsum.photos/200","state":"Arunachal Pradesh","city":"Pasighat"},{"id":654,"origin_station_code":1,"station_path":[53,67,74,89],"destination_station_code":92,"date":"03/21/2022 11:02 PM","map_url":"https://picsum.photos/200","state":"Arunachal Pradesh","city":"Pasighat"}];
  obj = [];
  obj_user = {};
  obj_upcoming = [];
  obj_past = [];
  obj_nearest = [];

    // rides2 = 
constructor(){

    super();
    // console.log("");
    this.state = {
        rides : []

    }
    
    }
   async componentDidMount() {
        let b = 0;
        let url = "https://assessment.api.vweb.app/rides";
        let url_user = "https://assessment.api.vweb.app/user";
        let data = await fetch(url);
        let data_user = await fetch(url_user);
        let parsedData_user = await data_user.json();
        
        let parsedData = await data.json();
        this.obj = parsedData;
this.obj_user = parsedData_user;
        // manipulating date
        for(let i = 0;i<this.obj.length;i++){
            // this.obj_upcoming.push(this.obj[i]);
            let d = this.obj[i].date.slice(0,10) +'';
            
            let d1 = new Date(d);
            if(d1>new Date()){
                this.obj_upcoming.push(this.obj[i]);
            }
            else{
                this.obj_past.push(this.obj[i]);
            }



        }
        // let arr = [];
        let j1=this.obj_user.station_code;
console.log(typeof(this.obj[0].station_path[0]));
        const s = new Set();
        for(let j = this.obj_user.station_code;j>0;j--){
            for(let k = 0;k<this.obj.length;k++){
                let st = this.obj[k].station_path+"";
                // console.log(st);
                let num = 0;
                for(let n = 0;n<st.length;n++){
                    if(st.charAt(n)===','){
                        num = num+1;
                    }
                }
                num = num+1;
                for(let l = 0;l<num;l++){
                
                    let c1 = j+"";
                    let c3 = j1+ "";
                    let c2 = this.obj[k].station_path[l]+"";
                    // console.log(c1+"="+c2+", "+ c3+"="+c2);

                    if(!s.has(k) && ((c1===c2)|| (c3===c2))){
                        this.obj_nearest.push(this.obj[k]);
                        s.add(k);
                        // break;
                    }
                
                }
            }
            j1 = j1+1;

        }
        for(let m = 0;m<this.obj.length;m++){
            // this.obj_upcoming.push(this.obj[i]);
            if(!s.has(m)){
                this.obj_nearest.push(this.obj[m]);
            }



        }

        // console.log(parsedData_user.name);
        this.setState({rides: this.obj_nearest})
//         fetch(
// "https://assessment.api.vweb.app/rides")
//             .then((res) => res.json())
//             .then((json) => {
//                 this.setState({
//                     rides: json,
//                     DataisLoaded: true
//                 });
//             })
    } 

     handleupcoming = async () => {
// console.log("upcoming");
this.setState({rides: this.obj_upcoming})


    }

     handlepast=async()=>{
        // console.log("past");
        this.setState({rides: this.obj_past})

    }
    
handlenearest= async() =>{
    // let url = "https://assessment.api.vweb.app/rides";
    // let url_user = "https://assessment.api.vweb.app/user";
    // let data = await fetch(url);
    // let data_user = await fetch(url_user);
    // let parsedData_user = await data_user.json();
    
    // let parsedData = await data.json();
    // console.log(parsedData_user.name);

    this.setState({rides: this.obj_nearest})
    // this.setState(rides)
}
  render() {
    // let {title,description,imageurl} = this.props;

    return (

        // nam = {this.obj_user.name}+"";
        <div className='' id='back'>
            {/* <NavBar/> */}
            <nav id='navbar' className="navbar">
         <div className="container" id='navb'>

           <a id='edvora' className="navbar-brand" href="/">Edvora</a>
           <div id='name'>{this.obj_user.name}  </div>
           <div ><img src={this.obj_user.url} id='img1' alt="" /></div>
       
         </div>
       </nav>
            {/* <h2>{this.obj_user.station_code} </h2>
            <h2>{this.obj_user.name}</h2> */}

            {/* <div className="d-flex justify-content-around"> */}
            <div id='buttons'>
            <a type="button" id='buttons' className="btn " onClick={this.handlenearest}><strong>Nearest rides ({this.obj_nearest.length})</strong></a>
            
            <a id='buttons' type="button" className="btn " onClick={this.handleupcoming}><strong>Upcoming Rides ({this.obj_upcoming.length})</strong></a>
            <a type="button" id='buttons' className="btn " onClick={this.handlepast}><strong>Past rides ({this.obj_past.length})</strong></a></div>
            {/* <a id='anchor' className="nav-link active" onClick={this.handlepast} style={{cursor:"pointer"}} aria-current="page">Active</a> */}

            {/* </div> */}

           
            {/* <NavBar/> */}
             {this.state.rides.map((element)=>{
                //  console.log(element.station_path +" abgu");
                 let ab ="["+ element.station_path +"]";
                 

                //  for(let p=0;0<element.station_path.length;p++){
                //      if(p===element.station_path.length-1){
                //          ab = ab+element.station_path[p];
                //      }
                //      else{
                //      ab = ab+element.station_path[p]+",";}

                //  }

                //  ab = ab+"]";
                
                 

                return         <div className='items my-2' key={element.date}>  <NewItems id = {element.id} originstation = {element.origin_station_code} stationpath = {ab} city = {element.city} state = {element.state} date = {element.date} imageurl= "data:image/jpeg;base64,"/>
                </div>

            })}
    

    </div>

    )
  }
}

export default News